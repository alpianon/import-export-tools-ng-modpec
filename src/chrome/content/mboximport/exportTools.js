...
//modPEC
// global object to create hash list of exported messages
var IEThashList = {
  file: null,
  total: 0,
  hashListArray: [],
...
// global object to create error list for SMIME verification
var IETSMIMEcheck = {
  file: null,
  total: 0,
  SMIMEcheckArray: [],
	...
	*/
	...
	switch (type) {
		...
		case 2:
			...
			break;
		case 3:
//modPEC			saveMsgAsEML(msguri,file,true,emlsArray,null,null,false,false,null,null,false);
//modPEC
			break;
		case 4:
			if (isMbox(file) !== 1) {
...
//modPEC
		...
		case 9:
			...
			break;
		//modPEC:
		case 10: // export selected messages with hash list and SMIME verification
			IETrepairedMessages = 0;
...
function exportAllMsgsDelayedVF(type, file, msgFolder) {
	...
	var datedir = buildContainerDirName();
	var useContainer = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.use_container_folder");

	//modPEC
	if (useContainer || type == 10) {
	//modPEC
...
function exportAllMsgsDelayed(type, file, msgFolder) {
	...
	var datedir = buildContainerDirName();
	var useContainer = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.use_container_folder");
	var skipExistingMsg = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.skip_existing_msg");
	...
	while (msgArray.hasMoreElements()) {
		...
		//modPEC
		if (! useContainer && skipExistingMsg && type != 10) {
		//modPEC
...
function IETrunExport(type, subfile, hdrArray, file2, msgFolder) {
	...
	switch (type) {
		...
		case 9: // plain text format, with index and attachments
			...
			break;
		//modPEC
		case 10: // export all messages with hash list and SMIME verification
			IETrepairedMessages = 0;
      IEThashList.total = IETtotal;
			IETSMIMEcheck.total = IETtotal;
...
function createIndex(type, file2, hdrArray, msgFolder, justIndex, subdir) {
	if (!IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.use_container_folder") && !justIndex && subdir)
		return;
...
function createIndexCSV(type, file2, hdrArray, msgFolder, addBody) {
	...
	var sep = IETprefs.getCharPref("extensions.importexporttoolsngmodpec.csv_separator");
	var data = "";
...
function exportAsHtml(uri, uriArray, file, convertToText, allMsgs, copyToClip, append, hdrArray, file2, msgFolder, saveAttachments) {
	...
	var myTxtListener = {
		...
		onAfterStopRequest: function (clone, data, saveAttachments) {
			...
			if (this.append && convertToText) {
				...
				data = data + "\r\n\r\n" + IETprefs.getCharPref("extensions.importexporttoolsngmodpec.export.mail_separator") + "\r\n\r\n";
	...
	var HTMLasView = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.HTML_as_displayed");
	...
	*/
	...
	var useConverter = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.use_converter");
...
function IETcopyToClip(data) {
	...
	var justText = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.clipboard.always_just_text");
...
function IETwritestatus(text) {
	if (document.getElementById("statusText")) {
		...
		var delay = IETprefs.getIntPref("extensions.importexporttoolsngmodpec.delay.clean_statusbar");
		if (delay > 0)
...
function IETwriteDataOnDisk(file, data, append, fname, time) {
	...
	foStream.close();
	if (time && IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.set_filetime"))
		file.lastModifiedTime = time;
}

function IETwriteDataOnDiskWithCharset(file, data, append, fname, time) {
	var os;
	var charset = IETprefs.getCharPref("extensions.importexporttoolsngmodpec.export.text_plain_charset");
...
function IETstoreHeaders(msg, msguri, subfile, addBody) {
	var subMaxLen = IETprefs.getIntPref("extensions.importexporttoolsngmodpec.subject.max_length") - 1;
	var authMaxLen = IETprefs.getIntPref("extensions.importexporttoolsngmodpec.author.max_length") - 1;
	var recMaxLen = IETprefs.getIntPref("extensions.importexporttoolsngmodpec.recipients.max_length") - 1;
	var realsubject;
	var author;
	var recipients;
	var body;
