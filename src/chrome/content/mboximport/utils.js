...
function getPredefinedFolder(type) {
	...
	switch (type) {
		case 0:
			use_dir = "extensions.importexporttoolsngmodpec.exportMBOX.use_dir";
			dir_path = "extensions.importexporttoolsngmodpec.exportMBOX.dir";
			break;
		case 1:
			use_dir = "extensions.importexporttoolsngmodpec.exportEML.use_dir";
			dir_path = "extensions.importexporttoolsngmodpec.exportEML.dir";
			break;
		default:
			use_dir = "extensions.importexporttoolsngmodpec.exportMSG.use_dir";
			dir_path = "extensions.importexporttoolsngmodpec.exportMSG.dir";
	}
	if (!IETprefs.getBoolPref(use_dir))
		return null;
	try {
    //modPEC
		var dirPathValue = IETgetComplexPref(dir_path,Components.interfaces.nsISupportsString);
    //modPEC
...
function getSubjectForHdr(hdr, dirPath) {
	var emlNameType = IETprefs.getIntPref("extensions.importexporttoolsngmodpec.exportEML.filename_format");
	var mustcorrectname = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.filenames_toascii");
	var cutSubject = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.cut_subject");
	var cutFileName = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.cut_filename");
  //modPEC
	var cutFileNameAt = IETprefs.getIntPref("extensions.importexporttoolsngmodpec.export.cut_filename_at");
  //modPEC
	...
	if (emlNameType === 2) {
		...
		if (IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.filename_add_prefix")) {
                        //modPEC
			var prefix = IETgetComplexPref("extensions.importexporttoolsngmodpec.export.filename_prefix",Components.interfaces.nsISupportsString);
                        //modPEC
...
function dateInSecondsTo8601(secs) {
	var addTime = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.filenames_addtime");
	...
	if (addTime && IETprefs.getIntPref("extensions.importexporttoolsngmodpec.exportEML.filename_format") === 2) {
...
function IETexport_all(just_mail) {
	if ((IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export_all.warning1") && !just_mail) || (IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export_all.warning2") && just_mail)) {
		...
		if (just_mail)
			IETprefs.setBoolPref("extensions.importexporttoolsngmodpec.export_all.warning2", !check.value);
		else
			IETprefs.setBoolPref("extensions.importexporttoolsngmodpec.export_all.warning1", !check.value);
		if (!result)
			return;
	}
...
function IETformatWarning(warning_type) {
	if (warning_type === 0 && !IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.format_warning"))
		return true;
	if (warning_type === 1 && !IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.import_warning"))
		return true;
	var prompts = Cc["@mozilla.org/embedcomp/prompt-service;1"]
		.getService(Ci.nsIPromptService);
	var check = { value: false };
	...
	if (warning_type === 0) {
		...
		pref = "extensions.importexporttoolsngmodpec.export.format_warning";
	} else {
		text = mboximportbundle.GetStringFromName("formatWarningImport");
		pref = "extensions.importexporttoolsngmodpec.export.import_warning";
	}
...
function IETremoteWarning() {
	if (!IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.remote_warning"))
		return true;
	var prompts = Cc["@mozilla.org/embedcomp/prompt-service;1"]
		.getService(Ci.nsIPromptService);
	var check = { value: false };
	...
	IETprefs.setBoolPref("extensions.importexporttoolsngmodpec.export.remote_warning", !check.value);
	return result;
}
...
function IETstr_converter(str) {
	var convStr;
	try {
		var charset = IETprefs.getCharPref("extensions.importexporttoolsngmodpec.export.filename_charset");
		if (charset === "")
			return str;
...
function nametoascii(str) {
	if (!IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.filenames_toascii")) {
...
var IETlogger = {
	write: function (string) {
		if (!IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.log.enable"))
			return;
		if (!IETlogger.file) {
			IETlogger.file = Cc["@mozilla.org/file/directory_service;1"]
				.getService(Ci.nsIProperties)
				.get("ProfD", Ci.nsIFile);
			IETlogger.file.append("ImportExportToolsNGmodPEC.log");
		}
		var now = new Date();
		var foStream = Cc["@mozilla.org/network/file-output-stream;1"]
			.createInstance(Ci.nsIFileOutputStream);
