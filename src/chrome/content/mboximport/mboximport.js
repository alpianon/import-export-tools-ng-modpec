...
var IETprintPDFmain = {
	...
	print: function (allMessages) {
		...
		try {
			if (IETprefs.getPrefType("print.always_print_silent") === 0 || !IETprefs.getBoolPref("print.always_print_silent")) {
				...
				IETprefs.setBoolPref("extensions.importexporttoolsngmodpec.printPDF.restore_print_silent", true);
			}
		} catch (e) { }
		// cleidigh check?
		IETabort = false;
	...
	print2: function () {
		...
		if (IETprefs.getIntPref("extensions.importexporttoolsngmodpec.printPDF.fileFormat") === 2)
		...
		IETprefs.setBoolPref("extensions.importexporttoolsngmodpec.printPDF.start", true);
		var messageList = [uri];
...
function trytocopyMAILDIR() {
	...
	var msgFolder = GetSelectedMsgFolders()[0];
	var buildMSF = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.import.build_mbox_index");
	...
	if (IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.import.name_add_number"))
	...
	if (!scandir) {
		...
		if (res === nsIFilePicker.returnOK) {
			...
			for (var i = 0; i < filesArray.length; i++) {
				...
				if (isMbox(afile) === 1) {
					var ask = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.confirm.before_mbox_import");
					if (ask) {
						...
						IETprefs.setBoolPref("extensions.importexporttoolsngmodpec.confirm.before_mbox_import", !checkObj.value);
...
function findGoodFolderName(foldername, destdirNSIFILE, structure) {
	var overwrite = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.export.overwrite");
	var index = 0;
	var nameIndex = "";
...
function writeDataToFolder(data, msgFolder, file, removeFile) {
	...
	else if (IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.reset_mozilla_status")) {
	...
	if (res === nsIFilePicker.returnOK) {
		...
		xmlhttp.onreadystatechange = function () {
			if (xmlhttp.readyState === 4) {
				...
				var subOn = IETprefs.getBoolPref("extensions.importexporttoolsngmodpec.sms.add_subject");
